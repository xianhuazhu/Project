...
-(NSString *)songTitle{
    songData *mode=[SongListHandle sharedHandle].songArr[_playIndex];
    return mode.kName;
}
...
-(void)setPlayIndex:(NSInteger)playIndex{
    if (playIndex!=-1) {
        ...
        //初始化音乐播放器
        //1.取出模型
        songData *mode=[SongListHandle sharedHandle].songArr[playIndex];
        //解析歌词
...
//下一首
-(void)nextSong{
    if(_playIndex<[SongListHandle sharedHandle].songArr.count-1){
        self.playIndex+=1;
    }else{
        self.playIndex=0;
    }
...
-(void)PrevousSong{
    if (_playIndex>0) {
        ...
        self.playIndex=[SongListHandle sharedHandle].songArr.count-1;
    }
}
...
-(void)audioPlayerDidFinishPlaying:(AVAudioPlayer *)player successfully:(BOOL)flag{
    ...
    if (flag) {
        switch (self.playMode) {
            ...
            case Random:
                //生成一个随机数 [0-count)
                self.playIndex=arc4random()%[SongListHandle sharedHandle].songArr.count;
                break;
            default:
                break;
        }
